...
> `graphify-out/`은 `.gitignore` 대상이라(2026-08-23부터, PR마다 그래프 스냅샷이 충돌하던
> 문제를 예방하기 위해 커밋 자체를 중단함) **정상 상태에서는** 이 계획에 등장하지 않는다.
> `git status`에 그냥 `M`으로 나타나면 훅이 만든 로컬 재생성 결과이니 무시한다. 단
> **`git rm --cached`로 언트랙되는 전환 순간 그 자체**는 다르다 — `D`(삭제) 상태로 잡히는
> 그 변경은 진짜 의미 있는 diff이므로 무시하지 않고 관련 `.gitignore`/스킬 문서 변경과
> 같은 그룹에 넣어 커밋한다(이 커밋 자체가 그 사례다).
...
- 모든 파일이 정확히 한 그룹에 속하는가 (누락·중복 없음, `git rm --cached`로 staged된 삭제
  포함 — `.gitignore` 전환 같은 작업에서 빠뜨리기 쉽다)
...
> `graphify-out/`은 `.gitignore` 대상이라 푸시 후에도 신경 쓰지 않는다(2026-08-23부터).
> 백그라운드 훅이 이 디렉터리를 계속 재생성해도 커밋 대상이 아니다.
