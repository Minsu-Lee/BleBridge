...
5. `--auto` 모드는 되묻지 않고 staged 범위를 1커밋으로 만든다. 따라서 **분리는 호출하는 쪽이
   스테이징으로 보장**한다 — 에이전트는 테스트만 stage해서 한 번, 구현만 stage해서 한 번,
   총 2회 호출한다. `--auto`로 테스트+구현이 섞여 들어오면 커밋은 하되 보고에 경고를 남긴다.
