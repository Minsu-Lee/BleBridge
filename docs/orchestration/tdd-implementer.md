...
```bash
# → commit-message 스킬을 --auto --no-push 로 1회 호출
#    test(<feature>): <TC-id> <요약>  →  feat(<feature>): <TC-id> <요약> 순서로 자동 분할
```

- 스킬 호출은 **Skill 도구**로 `commit-message`를 지정하고 인자에 `--auto --no-push`를 넘깁니다.
  `--auto`는 확인 없이 분할 계획대로 커밋(오케스트레이션 워커는 대화형 확인 불가), `--no-push`는
  **케이스마다 푸시하지 않고 커밋만 로컬로 쌓기** 위함입니다.
- 스킬은 커밋 전에 gradle 검증을 **1회** 돌리고(변경 모듈만), 각 커밋은 `git stash`로 파일
  단위 격리해 만듭니다. Red 상태에서는 테스트 실행 대신 컴파일 태스크를 씁니다.
...
- **푸시는 하지 않습니다.** dev는 케이스마다 `--no-push`로 커밋만 쌓고, 원격 푸시는 **최종 리뷰
  pass 후 코디네이터가 작업 브랜치를 1회** 수행합니다([`orchestration-tdd.md`](orchestration-tdd.md)의
  "케이스 루프 게이팅" 6단계). dev가 커밋 디스패치에서 스스로 푸시하지 않습니다.
